...
Параметры запуска моделей здесь приведены как базовый профиль после установки. Подробное обоснование TP/PP, NCCL и NVMe swap находится в [vllm.md](./vllm.md).
...
Hugepages снижают количество промахов TLB для CPU-тензоров PyTorch при загрузке и разгрузке моделей.

Расчёт для системы 256 ГБ RAM:
- HiCache L2: 30 ГБ × 4 ранга = 120 ГБ
- Остаток: ~136 ГБ (ОС, PyTorch, служебные процессы)
- Hugepages: 48 ГБ (~35% от остатка)
